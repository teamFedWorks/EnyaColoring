...
# #             _, _, out_frame = gfpgan_restorer.enhance(out_frame, has_aligned=False, only_center_face=False, paste_back=True, weight=weight)
# #     if use_realesrgan and realesrgan_upsampler is not None:
# #         # suppress Real-ESRGAN prints
# #         with contextlib.redirect_stdout(None), contextlib.redirect_stderr(None):
...
# def process_images(input_path, output_path, gfpgan_restorer, realesrgan_upsampler, weight, use_gfpgan, use_realesrgan, ext='auto', suffix=None):
...
#         out_img = process_frame(img, gfpgan_restorer, realesrgan_upsampler, weight, use_gfpgan, use_realesrgan)
...
#             save_ext = ext
#         if suffix:
#             out_file = os.path.join(output_path, f"{basename}_{suffix}.{save_ext}")
#         else:
#             out_file = os.path.join(output_path, f"{basename}.{save_ext}")
#         imwrite(out_img, out_file)

# def process_video(input_video, output_video, gfpgan_restorer, realesrgan_upsampler, weight, use_gfpgan, use_realesrgan):
...
#         if not ret:
#             break
#         out_frame = process_frame(frame, gfpgan_restorer, realesrgan_upsampler, weight, use_gfpgan, use_realesrgan)
#         out.write(out_frame)
#         pbar.update(1)
...
#     parser.add_argument('-i', '--input', type=str, required=True, help='Input image/video or folder')
#     parser.add_argument('-o', '--output', type=str, required=True, help='Output folder or video path')
#     parser.add_argument('--use_gfpgan', action='store_true', help='Enable GFPGAN face restoration')
#     parser.add_argument('--gfpgan_version', type=str, default='1.3', help='GFPGAN version: 1 | 1.2 | 1.3')
#     parser.add_argument('--use_realesrgan', action='store_true', help='Enable Real-ESRGAN upscaling')
#     parser.add_argument('--weight', type=float, default=0.5, help='GFPGAN enhancement weight')
#     parser.add_argument('--bg_tile', type=int, default=400, help='Tile size for Real-ESRGAN')
#     parser.add_argument('--suffix', type=str, default=None, help='Suffix for output images')
#     parser.add_argument('--ext', type=str, default='auto', help='Output image extension')
#     args = parser.parse_args()

#     # --- GFPGAN setup ---
#     if args.use_gfpgan:
#         if args.gfpgan_version == '1':
#             arch, channel_multiplier, model_name, url = 'original', 1, 'GFPGANv1', 'https://github.com/TencentARC/GFPGAN/releases/download/v0.1.0/GFPGANv1.pth'
#         elif args.gfpgan_version == '1.2':
#             arch, channel_multiplier, model_name, url = 'clean', 2, 'GFPGANCleanv1-NoCE-C2', 'https://github.com/TencentARC/GFPGAN/releases/download/v0.2.0/GFPGANCleanv1-NoCE-C2.pth'
#         else:
#             arch, channel_multiplier, model_name, url = 'clean', 2, 'GFPGANv1.3', 'https://github.com/TencentARC/GFPGAN/releases/download/v1.3.0/GFPGANv1.3.pth'
#         model_path = os.path.join('experiments/pretrained_models', model_name + '.pth')
#         if not os.path.isfile(model_path):
#             model_path = url
#         gfpgan_restorer = GFPGANer(model_path=model_path, upscale=2, arch=arch, channel_multiplier=channel_multiplier)
#     else:
...
#         realesrgan_upsampler = RealESRGANer(scale=2,
#                                             model_path='https://github.com/xinntao/Real-ESRGAN/releases/download/v0.2.1/RealESRGAN_x2plus.pth',
#                                             model=model, tile=args.bg_tile, tile_pad=10, pre_pad=0, half=True)
#     else:
#         realesrgan_upsampler = None

#     # --- Detect if input is video or images ---
#     if os.path.isfile(args.input) and args.input.lower().endswith(('.mp4', '.mov', '.avi', '.mkv')):
#         process_video(args.input, args.output, gfpgan_restorer, realesrgan_upsampler, args.weight, args.use_gfpgan, args.use_realesrgan)
#     else:
#         process_images(args.input, args.output, gfpgan_restorer, realesrgan_upsampler, args.weight, args.use_gfpgan, args.use_realesrgan, args.ext, args.suffix)
